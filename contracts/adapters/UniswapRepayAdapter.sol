...
contract UniswapRepayAdapter is BaseUniswapAdapter {
  ...
  function swapAndRepay(
    ...
    uint256 currentDebt = IKIP7(debtToken).balanceOf(msg.sender);
    ...
    IKIP7(debtAsset).safeApprove(address(LENDING_POOL), 0);
    IKIP7(debtAsset).safeApprove(address(LENDING_POOL), amountToRepay);
  ...
  function _swapAndRepay(
    ...
    IKIP7(debtAsset).safeApprove(address(LENDING_POOL), 0);
    IKIP7(debtAsset).safeApprove(address(LENDING_POOL), amount);
    uint256 repaidAmount = IKIP7(debtAsset).balanceOf(address(this));
    LENDING_POOL.repay(debtAsset, amount, rateMode, initiator);
    repaidAmount = repaidAmount.sub(IKIP7(debtAsset).balanceOf(address(this)));
    ...
    IKIP7(debtAsset).safeApprove(address(LENDING_POOL), 0);
    IKIP7(debtAsset).safeApprove(address(LENDING_POOL), amount.add(premium));
  }
