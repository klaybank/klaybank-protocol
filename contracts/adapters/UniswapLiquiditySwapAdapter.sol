...
contract UniswapLiquiditySwapAdapter is BaseUniswapAdapter {
  ...
  function swapAndDeposit(
    ...
    for (vars.i = 0; vars.i < assetToSwapFromList.length; vars.i++) {
      ...
      vars.bTokenInitiatorBalance = IKIP7(vars.bToken).balanceOf(msg.sender);
      ...
      // Deposit new reserve
      IKIP7(assetToSwapToList[vars.i]).safeApprove(address(LENDING_POOL), 0);
      IKIP7(assetToSwapToList[vars.i]).safeApprove(address(LENDING_POOL), vars.receivedAmount);
  ...
  function _swapLiquidity(
    ...
    vars.bTokenInitiatorBalance = IKIP7(vars.bToken).balanceOf(initiator);
    ...
    // Deposit new reserve
    IKIP7(assetTo).safeApprove(address(LENDING_POOL), 0);
    IKIP7(assetTo).safeApprove(address(LENDING_POOL), vars.receivedAmount);
    ...
    // Repay flash loan
    IKIP7(assetFrom).safeApprove(address(LENDING_POOL), 0);
    IKIP7(assetFrom).safeApprove(address(LENDING_POOL), vars.flashLoanDebt);
  }
