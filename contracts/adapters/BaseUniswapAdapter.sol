...
abstract contract BaseUniswapAdapter is FlashLoanReceiverBase, IBaseUniswapAdapter, Ownable {
  ...
  function _swapExactTokensForTokens(
    ...
    IKIP7(assetToSwapFrom).safeApprove(address(UNISWAP_ROUTER), 0);
    IKIP7(assetToSwapFrom).safeApprove(address(UNISWAP_ROUTER), amountToSwap);
  ...
  function _swapTokensForExactTokens(
    ...
    // Approves the transfer for the swap. Approves for 0 first to comply with tokens that implement the anti frontrunning approval fix.
    IKIP7(assetToSwapFrom).safeApprove(address(UNISWAP_ROUTER), 0);
    IKIP7(assetToSwapFrom).safeApprove(address(UNISWAP_ROUTER), maxAmountToSwap);
  ...
  function _getDecimals(address asset) internal view returns (uint256) {
    return IKIP7Detailed(asset).decimals();
  }
  ...
  function _pullBToken(
    ...
    if (_usePermit(permitSignature)) {
      IKIP7WithPermit(reserveBToken).permit(
        user,
        address(this),
    ...
    IKIP7(reserveBToken).safeTransferFrom(user, address(this), amount);
