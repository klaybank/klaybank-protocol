...
contract FlashLiquidationAdapter is BaseUniswapAdapter {
  ...
  function _liquidateAndSwap(
    ...
    vars.initCollateralBalance = IKIP7(collateralAsset).balanceOf(address(this));
    if (collateralAsset != borrowedAsset) {
      vars.initFlashBorrowedBalance = IKIP7(borrowedAsset).balanceOf(address(this));
    ...
    IKIP7(borrowedAsset).approve(address(LENDING_POOL), debtToCover);
    ...
    uint256 collateralBalanceAfter = IKIP7(collateralAsset).balanceOf(address(this));
    ...
    if (collateralAsset != borrowedAsset) {
      ...
      uint256 flashBorrowedAssetAfter = IKIP7(borrowedAsset).balanceOf(address(this));
    ...
    IKIP7(borrowedAsset).approve(address(LENDING_POOL), vars.flashLoanDebt);

    // Transfer remaining tokens to initiator
    if (vars.remainingTokens > 0) {
      IKIP7(collateralAsset).transfer(initiator, vars.remainingTokens);
    }
  }
