...
import {IncentivizedKIP7} from './IncentivizedKIP7.sol';
import {IKlaybankIncentivesController} from '../../interfaces/IKlaybankIncentivesController.sol';

/**
 * @title Klaybank KIP7 BToken
 * @dev Implementation of the interest bearing token for the Klaybank protocol
 * @author Aave
 */
contract BToken is
  VersionedInitializable,
  IncentivizedKIP7('BTOKEN_IMPL', 'BTOKEN_IMPL', 0),
  IBToken
{
  ...
  IKlaybankIncentivesController internal _incentivesController;
  ...
  function burn(
    ...
    IKIP7(_underlyingAsset).safeTransfer(receiverOfUnderlying, amount);
  ...
  function balanceOf(address user)
    public
    view
    override(IncentivizedKIP7, IKIP7)
    returns (uint256)
  {
   ...
   **/
  function totalSupply() public view override(IncentivizedKIP7, IKIP7) returns (uint256) {
  ...
  /**
   * @dev Returns the address of the Klaybank treasury, receiving the fees on this bToken
   **/
  function RESERVE_TREASURY_ADDRESS() public override view returns (address) {
    return _treasury;
  }
  ...
  /**
   * @dev For internal usage in the logic of the parent contract IncentivizedKIP7
   **/
  function _getIncentivesController() internal view override returns (IKlaybankIncentivesController) {
   ...
   **/
  function getIncentivesController() external view override returns (IKlaybankIncentivesController) {
  ...
  function transferUnderlyingTo(address target, uint256 amount)
    ...
    returns (uint256)
  {
    IKIP7(_underlyingAsset).safeTransfer(target, amount);
    return amount;
  }
